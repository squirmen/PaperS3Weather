...
void drawIcon(int x, int y, const uint8_t *icon, int dx, int dy,
              bool highContrast) {
...
void drawRSSI(int x, int y, int rssi) {
  ...
  if (quality >= 80)
    drawArc(x + 12, y, 16, 225, 315);
  if (quality >= 40)
    drawArc(x + 12, y, 12, 225, 315);
  if (quality >= 20)
    drawArc(x + 12, y, 8, 225, 315);
  if (quality >= 10)
    drawArc(x + 12, y, 4, 225, 315);
...
void drawHourlyForecast(int x, int y, int dx, int dy, int index) {
  ...
  canvas.drawString(formatTemp(currentWeather.hourly[index].temp), x + dx / 2,
                    y + 30);
  canvas.setTextDatum(TL_DATUM);
  ...
  int iconX = x + dx / 2 - 32;
  int iconY = y + 50;
  const uint8_t *weatherIcon =
      getWeatherIcon(currentWeather.hourly[index].weatherCode, isDay);
  drawIcon(iconX, iconY, weatherIcon, WEATHER_ICON_SIZE, WEATHER_ICON_SIZE,
           true);
}

void drawGraph(int x, int y, int dx, int dy, String title, int xMin, int xMax,
               float yMin, float yMax, float values[]) {
  String yMinString = String((int)yMin);
  String yMaxString = String((int)yMax);
  int textWidth = 5 + max(yMinString.length() * GRAPH_TEXT_WIDTH_FACTOR,
                          yMaxString.length() * GRAPH_TEXT_WIDTH_FACTOR);
  ...
  if (yMin < 0 && yMax > 0) {
    ...
    if (yPos > graphY && yPos < graphY + graphDY) {
      ...
      for (int xDash = graphX; xDash < graphX + graphDX - GRAPH_DASH_SPACING;
           xDash += GRAPH_DASH_SPACING) {
        canvas.drawLine(xDash, yPos, xDash + GRAPH_DASH_LENGTH, yPos,
                        TFT_BLACK);
      }
    }
  }
...
void drawTempGraph(int x, int y, int dx, int dy, String title, int xMin,
                   int xMax, float yMin, float yMax, float highValues[],
                   float lowValues[]) {
  String yMinString = String((int)yMin);
  String yMaxString = String((int)yMax);
  int textWidth = 5 + max(yMinString.length() * GRAPH_TEXT_WIDTH_FACTOR,
...
void drawCurrentConditions(int x, int y, int dx, int dy) {
  ...
  canvas.setTextDatum(TL_DATUM);
  canvas.drawLine(x, y + PANEL_TITLE_HEIGHT, x + dx, y + PANEL_TITLE_HEIGHT,
                  TFT_BLACK);
  ...
  const uint8_t *weatherIcon =
      getWeatherIcon(currentWeather.weatherCode, isDay);
  drawIcon(iconX, iconY, weatherIcon, WEATHER_ICON_SIZE, WEATHER_ICON_SIZE,
           true);
...
void drawSunInfo(int x, int y, int dx, int dy) {
  ...
  // Draw sunrise
  canvas.setTextSize(3);
  drawIcon(x + 25, y + 50, SUNRISE64x64, WEATHER_ICON_SIZE, WEATHER_ICON_SIZE,
           false);
  ...
  // Draw sunset
  drawIcon(x + 25, y + 125, SUNSET64x64, WEATHER_ICON_SIZE, WEATHER_ICON_SIZE,
           false);
  ...
  canvas.setTextDatum(TC_DATUM);
  String phaseText = "";
  if (moonPhase < MOON_PHASE_NEW_MIN || moonPhase > MOON_PHASE_NEW_MAX)
    phaseText = "New";
  else if (moonPhase < MOON_PHASE_WAXING_CRES)
    phaseText = "Waxing Cres";
  else if (moonPhase < MOON_PHASE_FIRST_QTR)
    phaseText = "First Qtr";
  else if (moonPhase < MOON_PHASE_WAXING_GIB)
    phaseText = "Waxing Gib";
  else if (moonPhase < MOON_PHASE_FULL)
    phaseText = "Full";
  else if (moonPhase < MOON_PHASE_WANING_GIB)
    phaseText = "Waning Gib";
  else if (moonPhase < MOON_PHASE_LAST_QTR)
    phaseText = "Last Qtr";
  else
    phaseText = "Waning Cres";
...
void drawWindInfo(int x, int y, int dx, int dy) {
  ...
  drawWindCompass(x + dx / 2, y + dy / 2 + 20, currentWeather.windDir,
                  currentWeather.windSpeed, COMPASS_RADIUS);
}
...
void drawM5PaperInfo(int x, int y, int dx, int dy) {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    canvas.setTextDatum(TC_DATUM);
    String statusText =
        (currentWeather.source == "MQTT") ? "MQTT updated" : "updated";
    canvas.drawString(statusText, x + dx / 2, y + 120);
    canvas.setTextDatum(TL_DATUM);
  }
  ...
  float displayTemp = (sensorTemp > SENSOR_ERROR_VALUE)
                          ? sensorTemp
                          : currentWeather.temperature;
  float displayHumid = (sensorHumid > SENSOR_ERROR_VALUE)
                           ? sensorHumid
                           : currentWeather.humidity;

  canvas.setTextSize(3);
  drawIcon(x + 35, y + 140, TEMPERATURE64x64, WEATHER_ICON_SIZE,
           WEATHER_ICON_SIZE, false);
  canvas.drawString(formatTemp(displayTemp), x + 35, y + 210);

  drawIcon(x + 145, y + 140, HUMIDITY64x64, WEATHER_ICON_SIZE,
           WEATHER_ICON_SIZE, false);
...
void displayWeather(bool partialUpdate) {
  ...
  // Draw main border
  canvas.drawRect(PANEL_BORDER, HEADER_HEIGHT, SCREEN_WIDTH - 28,
                  SCREEN_HEIGHT - 43, TFT_BLACK);

  // Draw top row panels (Current, Wind, Sun, M5Paper Info)
  canvas.drawRect(PANEL_SPACING, PANEL_TITLE_HEIGHT, SCREEN_WIDTH - 30, 251,
                  TFT_BLACK);
  ...
  drawGraph(PANEL_SPACING, 408, 232, 122, "UV Index", 0, 7, 0, 12,
            hourlyUVArray);
  drawGraph(247, 408, 232, 122, "Precip (%)", 0, 7, 0, 100, hourlyPrecipArray);
  drawGraph(479, 408, 232, 122, "Humidity (%)", 0, 7, 0, 100,
            hourlyHumidityArray);
  drawGraph(711, 408, 232, 122, "Pressure (hPa)", 0, 7, 980, 1040,
            hourlyPressureArray);
