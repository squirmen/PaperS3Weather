...
bool fetchWeatherData(float latitude, float longitude) {
  ...
  url += "&hourly=temperature_2m,precipitation_probability,relative_humidity_"
         "2m,pressure_msl,uv_index,weather_code";
  url += "&daily=temperature_2m_max,temperature_2m_min,precipitation_sum,"
         "relative_humidity_2m_mean,pressure_msl_mean,sunrise,sunset";
  url += useCelsius ? "&temperature_unit=celsius&wind_speed_unit=kmh&"
  ...
  for (int retry = 0; retry < HTTP_RETRY_ATTEMPTS; retry++) {
    ...
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (!error) {
        ...
        for (int i = 0;
             i < MAX_HOURLY && (currentHourOffset + i) < hourlyTemp.size();
             i++) {
          ...
          currentWeather.hourly[i].uvIndex =
              (apiIndex < hourlyUV.size()) ? hourlyUV[apiIndex].as<float>()
                                           : 0.0f;
    ...
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (!error && doc["results"].size() > 0) {
        ...
        Serial.printf("Found: %s, %s at %.4f, %.4f\n", foundCity.c_str(),
                      country.c_str(), latitude, longitude);
